...
def load_data():
    ...
    try:
        import os
        df = pd.read_csv(os.path.join(os.path.dirname(__file__), "wfh_fatigue_data.csv"))
...
def render_main_dashboard(df):
    ...
    time_filter = st.sidebar.selectbox("Select time range:", get_time_filter_options(), index=3)  # All Time
    ...
    if time_filter == "Custom Range":
        ...
        if filtered_df.empty:
            st.warning("⚠️ No data available in the selected time range. Try changing the time filter.")
            st.stop()
...
def render_analytics_page(df):
    ...
    with tab1:
        ...
        if 'Hour' in df.columns:
            ...
            st.plotly_chart(plot_hourly_heatmap(df), use_container_width=True, key="hourly_heatmap_analytics")
    ...
    with tab3:
        ...
        st.markdown("### Distribution of States")
        st.plotly_chart(fig, use_container_width=True)
        ...
        #     st.write("Common State Transitions:")
            ...
            #         <td style="padding: 8px; border-bottom: 1px solid #555;">{from_state}</td>
            #         <td style="padding: 8px; border-bottom: 1px solid #555;">{to_state}</td>
            #         <td style="padding: 8px; border-bottom: 1px solid #555;">{count}</td>
            #     </tr>
            #     """
            
            # transitions_html += "</table></div>"
    ...
    with tab4:
        ...
        st.plotly_chart(plot_hourly_heatmap(df), use_container_width=True, key="hourly_heatmap_dashboard")
